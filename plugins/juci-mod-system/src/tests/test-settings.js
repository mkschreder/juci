...
describe("Settings", function(){
	...
	it("should have required rpc functions", function(){
		expect($rpc.juci.system.password_set).to.be.a(Function); 
	}); 
	it("should be able to check for online upgrade", function(done){
		$rpc.juci.system.upgrade_check({type: "online"}).done(function(){
			done(); 
		}).fail(function(){
	...
	it("should be able to check for usb upgrade", function(done){
		$rpc.juci.system.upgrade_check({type: "usb"}).done(function(){
			done();  
		}).fail(function(){
			throw new Error("Could not check for online upgrade!"); 
		}); 
	...
	it("should not be able to set password without providing current password", function(done){
		...
		$rpc.juci.system.password_set({"user": PARAMS.username, "password": "abrakadabra"}).done(function(){
			// reset password to current password again!
			$rpc.juci.system.password_set({"user": PARAMS.username, "password": PARAMS.password, "curpass": "abrakadabra"}).always(function(){
	...
	it("should not be able to set password for root", function(done){
		if(PARAMS.username != "root"){
			$rpc.juci.system.password_set({"user": "root", "password": "foo"}).done(function(){
	...
	it("should be able to set password for current user", function(done){
		$rpc.juci.system.password_set({"user": PARAMS.username, "password": PARAMS.password, "curpass": PARAMS.password}).done(function(){
